...
YP.record = (function ($, tpl, RECORD_VARS) {
    ...
    record.compareRadio = function (option) {
        ...
        option = $.extend(param, option);
        var domObject = option.domOb, oldDataId = domObject.data("old"), oldData = domObject.find("input[value=" + oldDataId + "]").data("show"),
            newDataId = domObject.find("input[type=radio]:checked").val(), newData = domObject.find("input[type=radio]:checked").data("show"),
            title = domObject.data("title");
